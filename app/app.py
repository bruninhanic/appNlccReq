...
st.header('Percurso NLCC para a área requisitante')

st.subheader('Identique os próximos passos:')

st.caption('Qual a última etapa concluída?')
etapa = st.radio(
    'As etapas são sequenciais. Se você marcar uma delas, significa que a anterior foi concluída.',
    ('Vamos começar a planejar agora.',
    'Fase de Planejamento: Planejamento Anual de Compras e Lei Orçamentária Anual.',
    'Fase Preparatória: Estudo Técnico Preliminar, Termo de Referência ou Projeto Básico, Mapa de Riscos, Mapa de Preços, Designação Gestor/Fiscal do Contrato.',
    'Fase Externa: Publicação do Edital, Sessão de Julgamento, Habilitação e Homologação.',
    'Fase de Contratação: Assinatura da ata/contrato.')
)

if etapa == 'Vamos começar a planejar agora.':
    st.write('')
    ...
    if orcamento == 'Não':
        ...
        st.success('Veja a próxima etapa.')
 
elif etapa == 'Fase de Planejamento: Planejamento Anual de Compras e Lei Orçamentária Anual.':
        ...
        if finalEtp:
            ...
            if conclusaoEtp == 'Sim':
                    ...
                    st.text(
                        'Base Legal: Art. 23 da NLCC. Resolução Seplag n° 102, de 29 de dezembro de 2022.')
                    x += 1
                    st.write(
                        '{}. Providencie os documentos de designação do gestor e do fiscal do contrato.'.format(x))
                    ...
                    if finalDocumentos:
                        ...
                        if documentos == 'Sim':
                            st.write(
                                '{}. Encaminhe o processo para a área de compras, que promoverá a instrução processual, de acordo com o tipo de licitação adequado.'.format(x))
                            ...
                            if analiseCompras == 'Sim':
                                ...
                                st.write(
                                    '{}. Promova os ajustes solicitados pela área de compras e remeta o processo à área novamente.'.format(x))
                                x += 1

                            st.write(
                                '{}. Se não existir mais pendência, o processo será encaminhado para análise jurídica.'.format(x))

                            x += 1
                            finalJuridico = st.checkbox(
                            ...
                            if finalJuridico:
                                ...
                                if parecerJuridico == 'Sim':
                                    st.write(
                                        '{}. Promova os ajustes solicitados pelo parecer jurídico e remeta o processo à área de compras.'.format(x))
                                    x += 1

                                st.write(
                                    '{}. Se não existir mais pendência, o processo será encaminhado para publicação.'.format(x))
                                st.success('Edital Publicado! Veja a próxima etapa.')

elif etapa == 'Fase Preparatória: Estudo Técnico Preliminar, Termo de Referência ou Projeto Básico, Mapa de Riscos, Mapa de Preços, Designação Gestor/Fiscal do Contrato.':
    ...
    st.success('Processo de Compras homologado!')

elif etapa == 'Fase Externa: Publicação do Edital, Sessão de Julgamento, Habilitação e Homologação.':
    ...
    st.success('Ata/Contrato assinada/o! Veja a próxima etapa.')
    ...
    x = 1
    st.write(
    '{}. O empenho e a ordem de fornecimento/serviço podem ser emitidos.'.format(x))
    x += 1
